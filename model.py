...
def get_gemini_vision_response(input_prompt, image):
    """Generates a response from Gemini 2.5 Flash Vision model for image analysis."""
    model = genai.GenerativeModel('gemini-2.5-flash')
    try:
        with st.spinner("Trenzia is meticulously analyzing your image... ✨"):
...
def get_gemini_text_response(input_prompt):
    """Generates a response from Gemini 2.5 Flash model for text-based queries."""
    model = genai.GenerativeModel('gemini-2.5-flash')
    try:
        with st.spinner("Trenzia is curating the perfect insights for you... 🤔"):
...
st.set_page_config(
    page_title="Trenzia - Your Personal AI Stylist",
    page_icon="🌸",
    layout="wide",
...
st.sidebar.title("🌸 Trenzia Menu")
st.sidebar.markdown("Explore our exquisite fashion insights:")
page_selection = st.sidebar.radio(
    "Choose Your Path",
    ["Analyze My Style", "Fashion Trends", "Style Inspiration", "Virtual Try-On (Future)", "About Trenzia"]
)

st.sidebar.markdown("---")
st.sidebar.info("💡 **Tip**: Start your personalized fashion journey by selecting 'Analyze My Style' to receive instant insights!")
st.sidebar.markdown("Crafted with ❤️ by Trenzia Project Team.")

# --- Page Content Based on Selection ---

# --- Analyze My Style Page ---
if page_selection == "Analyze My Style":
    st.title("👗 Trenzia: Your Personal Style Analyzer")
    st.markdown("""
    Upload your photo or capture one with your camera, and let **Trenzia**, your AI stylist, delve into your look.
    ...
    with col2:
        ...
        if image_source is not None:
            ...
            occasion = st.selectbox(
                ...
                help="Selecting an occasion helps Trenzia provide more relevant and precise fashion advice."
            )

            st.subheader("AI Confidence Level:")
            confidence_level = st.slider(
                "How much creative freedom should Trenzia have? (Lower = more practical, Higher = more adventurous)",
            ...
            st.info(f"Trenzia's creative recommendation focus: **{int(confidence_level * 100)}%**")

            if st.button('🚀 Get Trenzia\'s Style Analysis & Recommendations', use_container_width=True, type="primary"):
                with st.spinner('Trenzia is meticulously analyzing your style and generating bespoke recommendations... This might take a moment. Please be patient.'):
                    image_for_api = Image.open(image_source)
                    
                    input_prompt = f"""
                    You are an extremely discerning, creative, and highly skilled personal fashion stylist named 'Trenzia' for the Fashion GeniAI project. Your ultimate goal is to provide a comprehensive, actionable, and profoundly inspiring fashion report based on the user's uploaded image.
                    ...
                    ---
                    ## 📝 Trenzia's Personalized Fashion Report
                    ...
                    ### **3. Trenzia's Transformative Recommendations for '{occasion}':**
                    Considering your unique current look and the selected '{occasion}', here are Trenzia's expert suggestions to truly elevate your ensemble:

                    -   **🎯 Core Style Enhancement**: Based on Trenzia's **{confidence_level}** confidence focus, propose the single most impactful adjustment or addition. This could be introducing a strategic layering piece, a particular type of footwear, or a bold statement accessory that ties the whole look together for the occasion.
                    ...
                    -   **🌟 The "Trenzia Spark" - Our Signature Touch**: Offer a unique, often-overlooked styling trick or a subtle detail that can dramatically transform the look (e.g., mastering the French tuck, rolling up sleeves just so, adding a pocket square, a unique way to layer necklaces). This is where your individual style truly shines.
                    ...
                    ---
                    **Remember**: Fashion is a powerful form of self-expression. Use Trenzia's insights as a sophisticated guide to boldly explore and confidently refine your personal style.
                    """
                    response_text = get_gemini_vision_response(input_prompt, image_for_api)
                    
                    if response_text:
                        st.subheader("✨ Trenzia's AI-Powered Style Report ✨")
            ...
            st.info("To unlock Trenzia's analysis and recommendations, please upload a photo or capture one using your camera in the left panel first.")

# --- Fashion Trends Page ---
elif page_selection == "Fashion Trends":
    st.title("📈 Trenzia: Discovering the Pulse of Fashion Trends")
    st.markdown("""
    Stay effortlessly ahead of the curve! Here, **Trenzia** curates the most impactful and exciting fashion trends for **2024-2025**.
    ...
    if st.button(f"Unveil '{selected_trend}' Insights", type="primary"):
        with st.spinner(f"Trenzia is meticulously researching '{selected_trend}' trends for you..."):
            trend_prompt = f"""
            You are an expert fashion journalist and trend forecaster for Fashion GeniAI, named 'Trenzia'.
...
# --- Style Inspiration Page ---
    ...
    st.title("💡 Trenzia: Your Wellspring of Style Inspiration")
    st.markdown("""
    Unleash your creativity and discover new dimensions of personal style!
    **Trenzia** provides curated inspiration from diverse themes, iconic celebrity looks, and seasonal aesthetics.
    """)
    st.markdown("---")
    ...
    if inspiration_type == "Style Theme":
        ...
        if st.button(f"Inspire Me with '{theme}' Style"):
            with st.spinner(f"Trenzia is generating captivating inspiration for '{theme}' style..."):
                inspiration_prompt = f"""
                You are an inspirational style consultant for Fashion GeniAI, named 'Trenzia'.
        ...
        if st.button(f"Analyze {celeb_name}'s Iconic Style"):
            if celeb_name:
                with st.spinner(f"Trenzia is meticulously analyzing {celeb_name}'s style legacy..."):
                    celeb_prompt = f"""
                    You are a highly knowledgeable celebrity stylist and fashion critic for Fashion GeniAI, named 'Trenzia'.
                    ...
                    ### **Trenzia's Insight: How to Get Inspired:**
        ...
        season = st.selectbox(
            ...
            help="Get Trenzia's curated fashion tips perfectly suited for each season."
        )
        if st.button(f"Unlock '{season}' Style Tips"):
            with st.spinner(f"Trenzia is crafting sophisticated style tips for '{season}'..."):
                seasonal_prompt = f"""
                You are a highly insightful seasonal fashion expert for Fashion GeniAI, named 'Trenzia'.
                ...
                if seasonal_info:
                    st.subheader(f"Trenzia's Guide to {season} Style")
                    st.markdown(seasonal_info)
                else:
                    st.warning("Could not generate seasonal tips. Please try again.")

    st.markdown("---")
    st.info("Let Trenzia ignite your imagination and transform your style journey!")
...
# --- Virtual Try-On (Future) Page ---
    ...
    **Trenzia's Virtual Try-On** feature is poised to redefine your fashion exploration and shopping experience.
...
# --- About Trenzia Page ---
    ...
    Trenzia harnesses the unparalleled capabilities of **Google Gemini's multi-modal AI**, specifically leveraging `gemini-2.5-flash`. This advanced model possesses an extraordinary aptitude for comprehending, interpreting, and generating insights from intricate visual information, enabling us to deliver fashion advice that is both highly accurate and profoundly relevant.
    
    ### **Our Grand Vision:**
    We passionately believe that fashion transcends mere clothing; it is a vibrant, empowering, and intrinsically joyful form of self-expression. **Trenzia** aspires to be your trusted confidante and indispensable partner in the exhilarating journey of discovering, refining, and celebrating your truly unique personal style.
    
    Thank you for becoming an integral part of the discerning **Trenzia** community! Your style revolution begins here.
    """)
    st.markdown("---")
    st.write("Trenzia is meticulously crafted with ❤️ passion, artistic vision, and relentless innovation. © 2025 Fashion GeniAI.")
